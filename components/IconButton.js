...
const IconButton = ({name = 'error', family = 'material', size = 32, title, lightColor, darkColor, color, style, textStyle, onPress}) => {
  ...
  return (
    <TouchableOpacity onPress={onPress} style={{ justifyContent: 'center', alignItems: 'center', ...style }}>
      <Icon name={name} family={family} color={themeColor} size={size} />
      {title &&
