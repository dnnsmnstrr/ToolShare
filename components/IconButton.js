...
const IconButton = ({name = 'error', family = 'material', size = 32, title, lightColor, darkColor, color, style, textStyle, onPress, focused, rounded = false}) => {
  const themeColor = useThemeColor({ light: lightColor || color, dark: darkColor || color }, 'text');
  const borderRadius = rounded ? size / 2 : null
  return (
    <TouchableOpacity onPress={onPress} style={{ justifyContent: 'center', alignItems: 'center', borderRadius, ...style }}>
